...
public class UnbindAction implements Action {
    ...
    private static final String URL_UNBIND = "/unbind";
    ...
    public BotApiMethod<Message> handle(Message message) {
        ...
        var textLogin = "Введите емейл и пароль через пробел, "
                + "чтобы отвязать аккаунт telegram от платформы CheckDev:";
    ...
    public BotApiMethod<Message> callback(Message message) {
        ...
        if (result.getPassword().equals(password) || result.getEmail().equals(email)) {
            ...
            try {
                mockCallWebClient
                        .doPostToUnbind(URL_UNBIND, email)
                        .block();
                text = "Ваш аккаунт отвязан от платформы CheckDev!";
                return new SendMessage(chatId, text);
            } catch (Exception e) {
